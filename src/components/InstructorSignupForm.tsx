...
const InstructorSignupForm: React.FC<signupFormProps> = ({ user, setUser, onSignup, buttonDisabled,setSignupAs }) => {
    ...
    return (
        <div className="bg-white px-8 py-12 rounded-3xl border-2 border-gray-200 w-full max-w-md relative">
            ...
            <p className="font-medium text-sm text-gray-500 mt-3 text-center">Instructor Signup Page</p>
            <div className="mt-6">
                <div>
                    <label htmlFor="" className="text-sm font-medium">User name</label>
                    <input name="userName" value={user.userName} onChange={handleChange} className="w-full border-2 border-gray-100 rounded-xl p-3 mt-1 bg-transparent text-sm" type="text" placeholder="Instructor Name" />
                </div>
                <div className="mt-4">
            ...
            <div className="flex items-center justify-center mt-6">
                <button onClick={()=>setSignupAs('student')} className="bg-violet-500 active:scale-[.98] active:duration-75 transition-all hover:scale-[1.01] ease-in-out text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline text-xs" type="button">
                    Sign up as student
                </button>
            </div>
        </div>
